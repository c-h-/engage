...
gulp.task("analyze:external", cb => {
  try {
    ...
    gutil.log(
      gutil.colors.red(
        "Analyze feature must be enabled for" +
          " pagespeed and lighthouse analysis to be generated."
      )
    );
    gutil.log(
      gutil.colors.cyan("Run `gulp enable analyze` to install dependencies.")
    );
  }
  if (ngrok) {
    startServer(() => {
...
gulp.task("analyze:weight", cb => {
  try {
    ...
    gutil.log(
      gutil.colors.red(
        "Web platform must be enabled for" +
          " dependency weight visualization to be generated."
      )
    );
    gutil.log(
      gutil.colors.cyan("Run `gulp enable web` to install web platform.")
    );
    cb();
  }
  if (wp) {
    try {
      ...
      gutil.log(
        gutil.colors.red(
          "Visualize feature must be enabled for" +
            " dependency weight visualization to be generated."
        )
      );
      gutil.log(
        gutil.colors.cyan("Run `gulp enable visualize` to install visualizer.")
      );
      cb();
    }
    if (vs) {
...
gulp.task("analyze:weight:open-report", cb => {
  openReport(
    path.join(process.cwd(), "build", "web", "bundle_weight_report.html"),
    cb
  );
});
...
gulp.task("analyze:external:lighthouse:open-report", cb => {
  openReport(
    path.join(process.cwd(), "build", "web", "progressive_web_app_report.html"),
    cb
  );
});
...
gulp.task("analyze:external:pagespeed:open-report", cb => {
  openReport(
    path.join(process.cwd(), "build", "web", "page_speed_report.html"),
    cb
  );
});
